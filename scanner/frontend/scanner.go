		...
		go func() {
			...
			for src := range work {
				func() {
					...
					domChecker := NewDOMInjectionChecker(s.cfg)
					findings, err := domChecker.Check(ctx, src)
					if err != nil {
					...
					secretChecker := NewSecretExposureChecker(s.cfg)
					secretFindings, err := secretChecker.Check(ctx, src)
					if err != nil {
						results <- workerResult{err: err}
						return
					}
					...
					cycleChecker := NewCycleChecker(s.cfg)
					cycleFindings, err := cycleChecker.Check(ctx, src)
					if err != nil {
						results <- workerResult{err: err}
						return
					}
